...
- Next, the multiplexed analog signal is demultiplexed and converted to digital. The tape data is demodulated (DCC uses 8-to-10 modulation) and errors are detected and corrected. This results in a PASC (MPEG-1 layer 1) data stream, which Philips calls "subband data". Also, the SYSINFO data is extracted which contains time codes, SCMS data and (for prerecorded tapes) text data. AUXINFO data (which contains track markers and song information) is demodulated and extracted from the ninth track.
- The PASC data is decoded and filtered to generate an IEC-60958 (a.k.a. IEC-958 a.k.a. S/PDIF) PCM signal. In the documentation this is sometimes called "baseband data".
...
- Optionally convert an analog signal to a digital PCM stream, unless a digital (S/PDIF) input is used
...
- The PASC stream is combined with SYSINFO data and error detection/correction codes are added. The result is modulated using 8-to-10 encoding, and multiplexed. This is called "tape formatting". The AUX data is also part of the multiplexed signal and may contain track markers. All 9 tracks can be recorded at the same time, or the 9th track can be recorded while the audio tracks are playing.
...
If you're new to the world of DCC, you will probably want to start by reading the datasheet for the SAA2023 or SAA3323 Drive Processor (DRP). They are essentially the same, except for the supply voltage (5V vs. 3.3V). DRPs were used in the last recorders that were produced by Philips, and combine a lot of functionality that was previously done by multiple chips. If you understand the functions of the DRP's, it's not too hard to understand the schematics of recorders of all generations, even though earlier recorders use older chips for which there is no datasheet.  
